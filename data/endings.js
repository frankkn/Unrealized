  ...
  UNREALIZED.endings = {
    ...
    full: [
      ...
      },
      {
        ...
        // 門檻用實際玩得到的分布定，不是用理論上限：五軸總和 >= 31（起始 25、上限 50）
        // 且沒有任何一軸掉到 4 以下。刻意兼顧的玩法拿得到 4–12%，
        // 而只顧單一軸的五種玩法全部是 0% —— 這正是要區分的東西。
        // （原本設成「4 軸 >= 7」，那是最佳化搜尋才到得了的水準，真人玩 5000 局只有 4 局達標。）
        ...
        when: function (state, h) {
          ...
          return total >= 31 && Math.min.apply(null, vals) >= 4;
        },
        title: '一樣都沒放掉',
