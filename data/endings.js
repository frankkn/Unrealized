(function (global) {
  ...
  UNREALIZED.endings = {
    ...
    full: [
      ...
      {
        // hard 層原本只有兩個出口，於是「全力上進」的玩法有 85% 拿到同一個結局。
        // 身體的判決是絕對的（這一點不改），但身體垮掉的人生不只一種寫法——
        // 倒下的時候旁邊有沒有人，是完全不同的兩件事。排在倒在辦公室前面。
        id: 'END_病房裡有人', tier: 'hard', tone: 'bittersweet', rarity: '少見',
        when: { attr: { key: 'health', op: '<=', value: 0 }, attrs: [{ key: 'bond', op: '>=', value: 7 }] },
        title: '病房裡有人',
        text: '身體是真的垮了，這件事沒有別的說法。但你住院那幾個月，床邊那張椅子沒有一天是空的——排班表是他們自己排的，你到後來才知道。你這輩子把自己燒得很兇，燒到最後才發現，有人一直在旁邊看著火，等著接你。'
      },
      {
        // hard 層的第四個出口。倒下的原因是「為了一個職稱」還是「為了一件你真的想做的事」，
        // 是完全不同的兩個故事，而遊戲原本只寫得出前者。排在倒在辦公室前面。
        id: 'END_那些年是我的', tier: 'hard', tone: 'bittersweet', rarity: '少見',
        when: { attr: { key: 'health', op: '<=', value: 0 }, attrs: [{ key: 'self', op: '>=', value: 7 }] },
        title: '那些年是我的',
        text: '身體是燒掉了，這件事你不會辯。但你很清楚自己是為了什麼燒的——不是為了一個職稱，不是為了讓誰閉嘴，是那件事你真的想做，而且你做成了。現在躺在這裡，你回頭數那幾年，數得出哪一年做了什麼。很多人到最後數不出來。你寧可是這樣。'
      },
      {
      ...
      },
      {
        ...
        title: '一樣都沒放掉',
        text: [
          { when: { flagsAll: ['有小孩'] }, text: '你沒有哪一項特別突出。錢夠用，工作說得過去，孩子還會回來吃飯，身體撐得住，而且這些年做的決定，多半是你自己想做的。要到很後來你才看懂，這比在某一件事上做到頂尖難得多——多數人是拿一樣去換另一樣，換到最後手上只剩一樣。你不是比較聰明，是每一次都選了比較難走的那條：兩邊都顧，兩邊都不做滿。' },
          { text: '你沒有哪一項特別突出。錢夠用，工作說得過去，週末有人約，身體撐得住，而且這些年做的決定，多半是你自己想做的。要到很後來你才看懂，這比在某一件事上做到頂尖難得多——多數人是拿一樣去換另一樣，換到最後手上只剩一樣。你不是比較聰明，是每一次都選了比較難走的那條：兩邊都顧，兩邊都不做滿。' }
        ]
      },
      {
        // ── 頂點層 ──
        // 這四個是刻意浮誇的天花板：條件都很硬（多半要某一軸真的頂到 9–10），
        // 排在「站上去了」前面，因為它們比它更窄。
        // 內容再大，語氣仍然照這個遊戲的規矩走：具體的細節、你放掉的東西、
        // 那一個瞬間——不然會讀起來像換了一款遊戲。
      ...
      },
      {
        ...
        // 遊戲有了含金湯匙的路線之後，更需要一個把它比下去的。
        id: 'END_那條巷子出來的', tier: 'whole', tone: 'good', rarity: '隱藏',
        when: function (state) {
          return (!!state.flags['單親'] || !!state.flags['勞動']) &&
            state.attrs.money >= 9 && state.attrs.achieve >= 8;
        },
        title: '那條巷子出來的',
        text: [
          // 條件是「單親 或 勞動」，但原本只寫了單親那一版——
          // 勞動出身是雙親健在的家庭，「你媽走得早」對他不成立
          { when: { flagsAll: ['單親'] }, text: '你買下的第一棟樓，站在頂樓看得到小時候那條巷子。那間鐵皮加蓋還在，現在租給別人。同輩裡沒有人料到是你——包括你自己，包括每一個當年說你這種家庭大概就這樣的老師。你沒有回去跟誰講過一句話，也沒有必要。你媽走得早，沒有看到這一天；她要是看到了，第一句大概還是問你有沒有吃飯。' },
          { text: '你買下的第一棟樓，站在頂樓看得到小時候那條巷子。家裡那間店的鐵捲門還在，招牌換過兩次了。同輩裡沒有人料到是你——包括你自己，包括每一個當年說你這種家庭大概就這樣的老師。你爸媽到很後來都還在問你「這樣會不會太冒險」，問到最後一次的時候，你已經買下第三棟了。' }
        ],
        limitedTo: undefined
      },
      {
      ...
      {
        // 起跳點高、而且真的把它變成了什麼。這是「開外掛」該有的樣子——
        // 不是輕鬆，是資源到位的時候你沒有把它揮霍掉。
        id: 'END_把牌打好了', tier: 'whole', tone: 'good', rarity: '稀有',
        when: function (state) {
          return (!!state.flags['富裕'] || !!state.flags['專業家庭']) &&
            state.attrs.achieve >= 7 && state.attrs.self >= 7 && state.attrs.bond >= 5;
        },
        title: '把牌打好了',
        text: '你一開始就拿到一手好牌，這件事你從來沒有假裝不是。有人一輩子在證明自己不需要那些幫忙，你沒有——你只是很早就決定，不要讓那手牌爛在自己手上。同輩裡跟你起點一樣的人，有幾個到現在還在花家裡的錢。你做的事情是你自己的，名字底下那些成績也是你自己的，而你從來沒有需要靠貶低起點來證明這件事。'
      },
      {
        // 富裕路線的另一種收尾：什麼都有，除了那個問題的答案
        id: 'END_那個姓氏', tier: 'flagCombo', tone: 'bittersweet', rarity: '少見',
        when: { flagsAny: ['富裕'], attr: { key: 'self', op: '<=', value: 3 } },
        title: '那個姓氏',
        text: '你這輩子沒有為錢煩惱過一天，這件事你很清楚有多少人求不到。只是每一次有人熱絡地靠過來，你腦子裡都會跑過同一個問題，而那個問題你到最後都沒有答案。你做過的每一個決定，好像都有人先幫你鋪過路——鋪得太平，平到你想不起來哪一段是自己走的。',
        limitedTo: undefined
      },
      {
      ...
      {
        // 同樣的問題：money>=6 太寬。配到股票是運氣，但要留得住得先賺得夠多。
        id: 'END_竹科的股票', tier: 'generation', tone: 'good', rarity: '少見', limitedTo: [1975],
        when: { generation: 1975, attr: { key: 'money', op: '>=', value: 8 }, attrs: [{ key: 'achieve', op: '>=', value: 5 }] },
        title: '竹科的股票',
      ...
      },
      {
        ...
        when: function (state) { return state.generation === 2005 && state.chapter >= 7 && (!!state.flags['丁客'] || !!state.flags['無子']) && state.attrs.bond <= 5; },
        title: '無子的晚年',
        text: [
          // 一個人做這個決定的版本。原本只有「你們兩個」那一版，
          // 而單身路線的「認真算過一個人生養小孩的可能」也會走到這裡
          { when: { flagsAll: ['無子'] }, text: '當年算過一個人生養小孩的可能，最後決定不要。現在你想的是，等你老到不能自己來，誰會是那個站出來的人。這個問題，你這代人比上一代更早想，也想得更清楚，但清楚不代表有答案。' },
          { text: '當年決定不生的時候，你想的是留給彼此更多空間。現在你想的是，等你們兩個都老到不能自己來，誰會是那個站出來的人。這個問題，你這代人比上一代更早想，也想得更清楚，但清楚不代表有答案。' }
        ]
      },
      {
        // 2005 原本只有「無子的晚年」一個專屬結局，1975 有四個、1990 有三個——
        // 那個世代在結局空間裡幾乎沒有自己的臉。補兩個，都長在推想層上。
        id: 'END_領不到的那一代', tier: 'generation', tone: 'bittersweet', rarity: '世代限定', limitedTo: [2005],
        when: function (state) {
          return state.generation === 2005 && state.chapter >= 7 &&
            state.attrs.money <= 2 && state.attrs.achieve >= 8;
        },
        title: '領不到的那一代',
        text: '你這輩子沒有一年偷懶過，勞保的級距也一路往上補。真的走到這一年才發現，那個數字跟你二十幾歲時看到的試算完全是兩回事。你不是唯一一個——你這屆的人在群組裡算來算去，算出來的結論都一樣：還得再做幾年。超高齡社會這個詞你聽了三十年，聽的時候一直以為講的是別人。'
      },
      {
        id: 'END_人做的那部分', tier: 'generation', tone: 'good', rarity: '世代限定', limitedTo: [2005],
        when: function (state) {
          return state.generation === 2005 && !!state.flags['被取代'] &&
            state.attrs.achieve >= 7 && state.attrs.self >= 6;
        },
        title: '人做的那部分',
        text: '二十幾歲那年你的入門工作被一套系統接手，你花了很久才不再覺得那是自己不夠好。後來你做的事情一直在往同一個方向移：機器做得比你快的，你就讓它做；剩下那些需要有人在場、需要有人負責、需要有人願意把話講難聽的，你留著。到這個年紀回頭看，你這一代人是最早被逼著回答「人到底還剩下什麼」的那一批，而你給出的答案，你自己認。'
      },
      {
      ...
      {
        // achieve>=6 曾經在有資格的 1990 局裡命中 83% —— 成就中位數是 8~9，
        // 等於「你是 1990」就會拿到這個結局，把後面所有更具體的結局全部蓋掉。
        // 「逆襲」要真的翻身：位置爬上去了，錢也跟上來了。
        id: 'END_22K的逆襲', tier: 'generation', tone: 'good', rarity: '少見', limitedTo: [1990],
        when: { generation: 1990, attr: { key: 'achieve', op: '>=', value: 8 }, attrs: [{ key: 'money', op: '>=', value: 5 }] },
        title: '22K的逆襲',
      ...
      },
      {
        ...
        when: { flagsAny: ['接案', '創業'], attr: { key: 'self', op: '>=', value: 6 }, attrs: [{ key: 'money', op: '>=', value: 4 }] },
        title: '那間店還開著',
      ...
      },
      {
        ...
        title: '家裡還很熱鬧',
        text: [
          { when: { flagsAll: ['有小孩'] }, text: '你這輩子沒賺到什麼大錢，職稱也就那樣，同學會上你通常是坐在旁邊聽別人講的那一個。可是每到過年，你家永遠是最多人的那一間。孩子會回來，老朋友還是會約，連隔壁鄰居都習慣進來坐一下再走。到這個年紀你才看懂，時間放在哪裡，哪裡就會長出東西來。' },
          { text: '你這輩子沒賺到什麼大錢，職稱也就那樣，同學會上你通常是坐在旁邊聽別人講的那一個。可是每到過年，你家永遠是最多人的那一間。姪子外甥從小在你這裡混，老朋友還是會約，連隔壁鄰居都習慣進來坐一下再走。到這個年紀你才看懂，時間放在哪裡，哪裡就會長出東西來。' }
        ]
      },
      {
    ...
    personalizations: [
      ...
      // 只掛在兩個分支旗標上，不掛「送走父母」——那個是人人有的，
      // 放進來會把上面所有專屬段落擠掉（這裡只取前兩個命中）
      { when: { flagsAll: ['來不及'] }, text: '你沒趕上最後那一面。這件事你後來很少提，但每年那一天都記得。' },
      { when: { flagsAll: ['撐住了'] }, text: '後事是你一件一件辦完的。等到全部結束、人都走了，你才坐下來哭。' },
      // 友誼那條線：朋友不會像家人一樣自動留在你的生活裡
      // 修復那條線：這是唯一一個「後來還是補回來了」的結尾
      { when: { flagsAll: ['修復過'] }, text: '四十幾歲那通電話之後，你們又處了二十年。前面那十年，現在講起來像是別人的事。' },
      { when: { flagsAll: ['沒有回頭'] }, text: '有幾個人你一直沒有再聯絡。不是不能，是每過一年就更難開口一點，然後就沒有然後了。' },
      // 手足那條線
      { when: { flagsAll: ['手足決裂'] }, text: '房子分完之後，兄弟姐妹就散了。逢年過節那個群組還在，但沒有人講話。' },
      { when: { flagsAll: ['手足還在'] }, text: '爸媽走了之後，你們反而更常聯絡。現在輪到你們是彼此最老的親人。' },
      { when: { flagsAll: ['手足沒出現'] }, text: '照顧那幾年只有你在。後來誰都沒提，你也就沒提。' },
      { when: { flagsAll: ['獨生'] }, text: '你這輩子沒有可以商量的人。所有的決定，都是你一個人在那個房間裡做的。' },
      { when: { flagsAll: ['家裡出頭期'] }, text: '那間房子的頭期款是家裡出的。這件事你沒有跟很多人講過，但你每次繳完貸款都會想起來一次。' },
      { when: { flagsAll: ['有靠山'] }, text: '第一年燒掉的那筆錢，家裡從來沒有提過要你還。你也一直沒問，那算不算一筆債。' },
      { when: { flagsAll: ['小留學生'] }, text: '十五歲那年的機場，你到現在都還記得。後來你去過很多機場，沒有一個像那一個。' },
      { when: { flagsAll: ['高期待'] }, text: '爸媽到最後都沒有說過「你做得很好」。你也是很後來才發現，自己一直在等那句話。' },
      { when: { flagsAll: ['休息過'] }, text: '三十歲那年停下來的那一整年，你這輩子提起它的次數，比提起任何一份工作都多。' },
      { when: { flagsAll: ['交情還在'] }, text: '那個你陪他熬過去的朋友，後來每年生日都記得打給你，一次都沒漏過。' },
      { when: { flagsAll: ['借錢給朋友'] }, text: '那筆借出去的錢一直沒回來，你們也一直沒提。但需要人的時候，他還是會出現。' },
      { when: { flagsAll: ['死黨'] }, text: '十八歲那個什麼都跟他說的人，你們現在一年見一次，坐下來還是三分鐘就回到當年。' },
      { when: { flagsAll: ['朋友走散'] }, text: '那群人你後來一個都沒有再聯絡。偶爾在臉書上看到誰的近況，也就看看而已。' }
    ],
